...
export function getLegalSecondPlacements(postFirstBoard, firstCell, player) { const out=[]; for(let i=0;i<BOARD_CELLS;i++) if(postFirstBoard[i]==null&&!isForbiddenSecondPlacement(postFirstBoard,firstCell,i,player)) out.push(i); return out; }
export function hasLegalFullTurn(turnStart) { const need=requiredPlacements(turnStart); if(need===1) return turnStart.board.some(x=>x==null); for(let i=0;i<BOARD_CELLS;i++) if(turnStart.board[i]==null){ const a=placementEffect(turnStart.board,turnStart.activePlayer,i); if(a.ok) for(let j=0;j<BOARD_CELLS;j++) if(a.board[j]==null&&!isForbiddenSecondPlacement(a.board,i,j,turnStart.activePlayer)) return true; } return false; }
